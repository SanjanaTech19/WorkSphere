                ...
                {data?.gamification && (
                    <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-[2.5rem] shadow-sm">
                            ...
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    ...
                                    <span className="text-[10px] text-zinc-400 font-mono">
                                        XP: {data.gamification.xp} / {data.gamification.xp - data.gamification.xpInCurrentLevel + data.gamification.xpForNextLevel}
                                    </span>
                                </div>
                                <h2 className="text-3xl font-black uppercase tracking-tighter">
                                    Level {data.gamification.level} <span className="text-blue-600">Workspace Scout</span>
                                </h2>
                            </div>
                            <div className="text-left md:text-right">
                                <p className="text-[9px] font-black text-zinc-400 uppercase tracking-widest mb-1">XP Breakdown</p>
                                <div className="flex flex-wrap gap-4 text-[10px] font-bold uppercase tracking-wider text-zinc-500">
                                    <span>Reviews: {data.gamification.xpBreakdown.reviewsXp} XP</span>
                                    <span>Venues: {data.gamification.xpBreakdown.venuesXp} XP</span>
                                    <span>Speedtests: {data.gamification.xpBreakdown.speedtestsXp} XP</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-between items-center mt-3 text-[10px] font-black uppercase tracking-widest text-zinc-400">
                            ...
                            <span>{data.gamification.xpForNextLevel - data.gamification.xpInCurrentLevel} XP to Level {data.gamification.level + 1}</span>
                            <span>LVL {data.gamification.level + 1}</span>
                        </div>
                    </div>
                )}
                            ...
                            <div className="bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 p-8 rounded-[2.5rem] shadow-sm space-y-6">
                                <div className="flex items-center justify-between mb-2">
                                    ...
                                    <span className="text-[10px] font-mono text-zinc-400">
                                        {data.gamification.badges.filter(b => b.earned).length} / {data.gamification.badges.length} Earned
                                    </span>
                                </div>
                                ...
                                <div className="grid grid-cols-3 gap-4">
                                    {data.gamification.badges.map((badge) => (
                                        <div key={badge.id} className="group relative flex flex-col items-center">
                                            ...
                                            <div className={`relative w-16 h-16 rounded-2xl flex items-center justify-center p-0.5 transition-all duration-300 group-hover:scale-105 ${
                                                badge.earned 
                                                ...
                                                <div className="w-full h-full rounded-[14px] bg-white dark:bg-zinc-955 flex items-center justify-center">
                                                    ...
                                                    {badge.icon === 'wifi' && (
                                                        <svg className="w-8 h-8 text-blue-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M12 20H12.01M8.5 16.5C9.5 15.5 10.75 15 12 15C13.25 15 14.5 15.5 15.5 16.5M5 13C7 11 9.5 10 12 10C14.5 10 17 11 19 13M1.5 9.5C4.5 6.5 8.25 5 12 5C15.75 5 19.5 6.5 22.5 9.5" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                                                        </svg>
                                                    )}
                                                    {badge.icon === 'cafe' && (
                                                        <svg className="w-8 h-8 text-emerald-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M17 8H19C20.1046 8 21 8.89543 21 10V12C21 13.1046 20.1046 14 19 14H17M3 8H17V14C17 17.3137 14.3137 20 11 20H9C5.68629 20 3 17.3137 3 14V8Z" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"/>
                                                            <path d="M6 2V5M10 2V5M14 2V5" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/>
                                                        </svg>
                                                    )}
                                                    {badge.icon === 'moon' && (
                                                        <svg className="w-8 h-8 text-purple-500" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                                                            <path d="M12 3C10.8251 4.17493 10.1762 5.76045 10.1762 7.41727C10.1762 9.07409 10.8251 10.6596 12 11.8345C10.2223 11.954 8.46823 11.3644 7.15949 10.2078C5.85074 9.05121 5.09341 7.42398 5.07019 5.71536C5.04698 4.00673 5.76001 2.36199 7.03714 1.1766C4.84656 2.01254 3.06456 3.65484 2.03666 5.78523C1.00876 7.91563 0.814324 10.3601 1.491 12.6457C2.16768 14.9312 3.6631 16.8778 5.68725 18.1077C7.7114 19.3377 10.1066 19.7523 12.41 19.2711C14.7134 18.79 16.7454 17.4514 18.1132 15.5146C19.481 13.5779 20.076 11.2037 19.7828 8.85236C19.4897 6.50106 18.3308 4.3648 16.5305 2.85764C14.7302 1.35048 12.4287 0.591079 10.072 0.725998C10.7485 1.40879 11.4552 2.16239 12 3Z" fill="currentColor"/>
                                                        </svg>
                                                    )}
                                                </div>
                                            </div>
                                            ...
                                            <div className="absolute bottom-20 scale-0 group-hover:scale-100 transition-all origin-bottom duration-200 bg-zinc-950 text-white text-[10px] p-4 rounded-xl shadow-2xl border border-zinc-800 w-44 z-30 pointer-events-none opacity-0 group-hover:opacity-100">
                                                <div className="font-black uppercase tracking-wider text-[9px] mb-1 flex items-center justify-between">
                                                    ...
                                                    <span className={`text-[7px] px-1.5 py-0.5 rounded font-mono ${
                                                        badge.earned ? 'bg-green-500/20 text-green-400' : 'bg-zinc-800 text-zinc-400'
                                                    }`}>
                                                        {badge.earned ? 'UNLOCKED' : 'LOCKED'}
                                                    </span>
                                                </div>
                                                <p className="text-zinc-400 font-bold leading-snug mb-2">{badge.description}</p>
